...
class AuthController extends Controller {
    ...
    public function register(Request $request, Response $response, array $args): Response {
        try {
            ...
            if (mb_strlen($username, 'utf8') < 3 || mb_strlen($username, 'utf8') > 35) throw new AuthException("Votre pseudo doit contenir entre 3 et 35 caractères.", "showRegister");
            ...
            if (User::where('username', '=', $username)->exists()) throw new AuthException("Ce pseudo est déjà pris.", "showRegister");
            ...
            $this->flash->addMessage('success', "$username, votre compte a été créé! Vous pouvez dès à présent vous connecter.");
            $response = $response->withRedirect($this->router->pathFor('showLogin'));
        } catch (AuthException $e) {
